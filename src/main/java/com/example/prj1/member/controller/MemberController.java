...
public class MemberController {
    ...
    public String edit(@RequestParam(required = false) String id, Model model, RedirectAttributes rttr) {
        if (id == null || id.isEmpty()) {
            ...
            return "redirect:/member/list"; // 목록 페이지 등으로 리다이렉트
        }
        model.addAttribute("member", memberService.get(id));
        return "member/edit";
    }
    ...
    public String changePassword(@ModelAttribute MemberForm data,
                                 RedirectAttributes rttr) {
        boolean ok = memberService.changePassword(data.getId(),
                data.getOldPassword(),
                data.getNewPassword());
        if (ok) {
